...
仅 admin。MVP 使用软删除并创建后续清理流程，成功返回 204。正式实现前必须确定隐私政策和恢复期限。
...
`format` 最终值待导出方案确认；MVP 草案使用 zip，其中包含 UTF-8 JSON/CSV 与可选照片。
...
仅发起宝宝的当前 admin。返回短时效签名 URL，并记录审计日志。
