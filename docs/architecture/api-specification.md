...
导出 pending/processing/failed 数、最老 pending 年龄、导出 worker 与媒体清理
scheduler 的 active/unhealthy 实例数及最近成功/失败心跳；不包含宝宝、用户、任务、
对象或 token 标识。
...
只能更新访问令牌对应用户，不接受客户端用户 ID。`displayName` trim 后必须为
1–80 个 Unicode 字符；成功返回 `UserSummary`。
...
已实现决策：图片完成时验证对象和尺寸，不另生成缩略图；导出 ZIP 同时提供
JSON 和 CSV 并可选照片；MVP 开放管理员删除宝宝接口，采用软删除、立即失权和后续物理清理。
