...
在 M2 的业务链路通过前，不要并行铺开时间轴、图表、邀请和导出等全部功能。
...
以下问题尚未最终确认，但不影响 M1 和 M2 的第一条业务链路：
...
- [ ] 已阅读必读文档。
- [ ] 确认只实现当前里程碑。
- [ ] Node、pnpm 和 Docker 版本已记录。
- [ ] 本地 PostgreSQL 和 MinIO 可启动。
- [ ] `.env.example` 不含真实秘密。
- [ ] staging/production 配置不会默认启用模拟登录。
- [ ] Prisma 入口和迁移策略已建立，尚未提前创建 M2 业务表。
- [ ] health 接口与 `api-specification.md` 的通用响应约定对齐。
- [ ] M2 所需的认证和成员守卫扩展点已保留。

## 14. 当前建议

下一个开发 Lead Agent 不需要继续做大规模需求分析，应该只执行 `current-milestone.md` 中的 M1。M1 验收完成后，再由 M2 实现模拟登录、宝宝创建、成员权限、幂等和对应测试。
