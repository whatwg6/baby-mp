...
- 校验 MIME、大小、对象存在性、媒体状态及宝宝归属；object key 不作为字段/接口返回，随机键不得包含业务信息，签名 URL 与键均不得记录日志。
- 实现三类记录创建、详情、更新和软删除。
...
- 验证签名 URL 短时有效、bucket 保持私有，日志不包含正文、宝宝姓名、object key 或签名 URL；临时上传键在确认后固化为不可覆盖 ready 对象，避免过期前覆盖。
...
- [x] M3 迁移可在 M2 数据库和空数据库前进执行。
- [x] 私有上传、完成确认、短期访问和未关联媒体清理可验证。
- [x] MIME、大小、对象状态和跨宝宝媒体校验符合契约。
- [x] 图文、测量和里程碑均可创建、查看、编辑和软删除。
- [x] 记录与媒体关联事务原子，重复提交不会产生重复记录。
- [x] admin/editor/viewer 与记录创建者权限矩阵通过。
- [x] 时间轴混合排序、筛选、游标分页和新增数据稳定性通过。
- [x] 删除/编辑记录后时间轴、详情和媒体关联保持一致。
- [x] 客户端上传失败保留表单，重试和切换宝宝不串数据。
- [x] 跨宝宝 mediaId/recordId 和 outsider 请求不泄漏资源存在性。
- [x] 对象 bucket 私有，签名地址短时有效且日志不记录敏感内容。
- [x] OpenAPI、共享 contracts、迁移与实现一致。
- [x] `pnpm verify` 和 M3 真实 API 验收通过。
- [x] H5 完整闭环通过，微信开发者工具完成适用验证。
- [x] `implementation-status.md` 记录实际证据、限制和 M4/M5 readiness。
