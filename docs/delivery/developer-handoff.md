...
| 当前状态 | M0–M6 完成；M7 仓库实现与本地验收完成，等待外部预发布/正式发布条件 |
| 更新日期 | 2026-07-22 |
...
M2–M6 已按依赖顺序完成登录与宝宝档案、私有媒体与三类记录、成长趋势、家庭协作和数据导出。M7 的仓库代码、自动化测试、本地 PostgreSQL/MinIO 验收、备份恢复和发布材料已经完成；当前只剩必须由平台账号、云环境、运营主体和真实设备提供的发布证据。

多人或多 agent 协作时，任务依赖、并行边界和文件所有权以 `docs/delivery/agent-workstreams.md` 为准。

## 2. 必读文档顺序

1. `docs/product/product-requirements.md`：MVP 做什么、不做什么。
2. `docs/product/information-architecture.md`：页面关系和用户流程。
3. `docs/product/ui-specification.md`：页面行为、字段和状态。
4. `docs/architecture/technical-architecture.md`：工程边界和安全要求。
5. `docs/architecture/data-model.md`：数据库实体、约束和索引。
6. `docs/architecture/api-specification.md`：HTTP 契约和错误码。
7. `docs/quality/test-plan.md`：必须覆盖的高风险路径。
8. `docs/delivery/development-plan.md`：里程碑和完成标准。
9. `docs/delivery/agent-workstreams.md`：多人并行开发时的拆分与合并顺序。
...
以下问题不影响仓库内 MVP 实现，但会阻塞 staging 或正式发布：
...
- 图片内容安全产品。
- 宝宝物理删除和账号注销的最终保留期限。
- 微信 AppSecret 与合法 request/upload/download 域名；AppID 已确定为 `wx433aecb90d44e9fe`。
- 监控告警通知渠道、当班责任人和升级链。
- 小程序运营主体、服务类目、隐私联系方式和最终保留政策。
...
- 以 `docs/architecture/api-specification.md` 为首版设计，代码完成后生成 OpenAPI 3 文件。
...
- 页面路由和 tab 结构遵循 `docs/product/information-architecture.md`。
...
- [x] M2–M6 全部业务域和实时成员授权已验证。
- [x] M1–M7 前进迁移、真实 PostgreSQL/MinIO 和导出 worker 已验证。
- [x] OpenAPI、共享 contracts、客户端运行时校验和生产构建保持同步。
- [x] 幂等、乐观版本、软删除、数据权利、低敏日志和依赖安全基线已验证。
- [x] 核心 H5 旅程及弱网、重复提交、上传中断、会话失效韧性旅程已通过。
- [ ] staging/production、微信体验版、双账号和 iOS/Android 真机证据待外部条件提供。

## 14. 当前建议

以 `docs/delivery/current-milestone.md` 中的 M7 为唯一当前验收基线。仓库接手者不应继续扩展 MVP 功能；应先锁定候选 commit，取得 AppSecret、正式 HTTPS 域名、云资源、运营信息和真机账号，然后按发布检查清单执行 staging、体验版、回滚和 production 观察。未取得真实外部证据前不得把 M7 或正式发布标记为完成。
